...
use dig_mempool::{
    ConfirmedBundleInfo, FeeEstimatorState, Mempool, MempoolConfig, SerializedBucket,
};
...
fn vv_req_fee_005_json_round_trip() {
    ...
    let restored: FeeEstimatorState =
        serde_json::from_str(&json).expect("deserialization from JSON must succeed");

    assert_eq!(
        restored.buckets.len(),
        state.buckets.len(),
        "bucket count must survive round-trip"
    );
    assert_eq!(
        restored.block_history.len(),
        state.block_history.len(),
        "history len must survive round-trip"
    );
    assert_eq!(
        restored.current_height, state.current_height,
...
fn vv_req_fee_005_estimation_preserved_after_round_trip() {
    ...
    for h in 0u64..12 {
        m1.record_confirmed_block(h, std::slice::from_ref(&bundle));
    }
...
fn vv_req_fee_005_includes_required_fields() {
    ...
    assert!(
        ...
        "FeeEstimatorState must include buckets"
    );
    assert_eq!(
        state.block_history.len(),
        1,
        "FeeEstimatorState must include block_history"
    );
    assert_eq!(
        state.current_height, 42,
        "FeeEstimatorState must include current_height"
    );
}
...
fn vv_req_fee_005_serialized_bucket_all_fields() {
    ...
    let restored: SerializedBucket =
        serde_json::from_str(&json).expect("SerializedBucket must deserialize from JSON");
...
fn vv_req_fee_005_empty_state_round_trip() {
    ...
    let state = m.snapshot_fee_state();
    assert!(
        state.block_history.is_empty(),
        "fresh tracker must have empty history"
    );

    let json = serde_json::to_string(&state).expect("empty state must serialize");
    let restored: FeeEstimatorState =
        serde_json::from_str(&json).expect("empty state must deserialize");
    assert!(restored.block_history.is_empty());
    assert!(
        !restored.buckets.is_empty(),
        "buckets must be preserved even in empty state"
    );
}
